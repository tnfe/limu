(function (global, factory) {
  ...
  var SHOULD_REASSIGN_MAP_METHODS = ['set', 'clear', 'delete'];
  ...
  var CAREFUL_FNKEYS = (_b = {},
      ...
      _b[ARRAY] = arrFnKeys,
      _b);
  var CHANGE_FNKEYS = (_c = {},
      _c[MAP] = ['clear', 'set', 'delete'],
      _c[SET] = ['clear', 'add', 'delete'],
      _c[ARRAY] = arrFnKeys,
      _c);
  var PROXYITEM_FNKEYS = (_d = {},
      _d[MAP] = ['forEach', 'get'],
      _d[SET] = ['forEach'],
      _d[ARRAY] = ['forEach', 'map'],
      _d);
  ...
  ***************************************************************************** */
  ...
  function getProxyVal(selfVal, options) {
      ...
      var mayCreateProxyVal = function (selfVal, inputKey) {
          ...
          if (parentType === SET) {
              ...
              replaceSetOrMapMethods(tmp_1, parentMeta, {
                  dataType: SET,
                  patches: patches,
              ...
              replaceSetOrMapMethods(tmp_2, parentMeta, {
                  dataType: MAP,
                  patches: patches,
                  inversePatches: inversePatches,
                  usePatches: usePatches,
  ...
  function replaceSetOrMapMethods(mapOrSet, meta, options) {
      ...
      if (dataType === SET) {
          ...
          mapOrSet.add = function limuAdd() {
              ...
              markModified(meta);
              recordPatch(__assign({ meta: meta }, options));
  ...
  function buildLimuApis(options) {
      ...
      var execOnOperate = function (op, key, parentMeta) {
          if (!parentMeta || !onOperate)
              return;
          var selfType = parentMeta.selfType, keyPath = parentMeta.keyPath, self = parentMeta.self, copy = parentMeta.copy;
          var fnKeys = CAREFUL_FNKEYS[selfType] || [];
          var isChange = op !== 'get';
          var isBuiltInFnKey = false;
          if (fnKeys.includes(key)) {
              isBuiltInFnKey = true;
              var changeFnKeys = CHANGE_FNKEYS[selfType] || [];
              isChange = changeFnKeys.includes(key);
          }
          onOperate({
              parentType: selfType,
              op: op,
              isBuiltInFnKey: isBuiltInFnKey,
              isChange: isChange,
              key: key,
              keyPath: keyPath,
      ...
      var limuApis = (function () {
          ...
          return {
              createDraft: function (mayDraft) {
                  ...
                  var meta = createScopedMeta(oriBase, {
                      key: '',
                      ver: metaVer,
                      traps: limuTraps,

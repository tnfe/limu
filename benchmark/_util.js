...
exports.showMem = function (label = 'Process', newLine = false) {
  ...
  const info = `${label}: heapTotal ${format(mem.heapTotal)} heapUsed ${format(mem.heapUsed)} rss ${format(mem.rss)} external ${format(
    mem.external,
  )}`;
...
function buildBase(arrLen = 10000) {
  const arr = Array(arrLen)
    .fill(0)
    .map((val, idx, arr) => (arr[idx] = { a: 1 }));
  return {
    arr,
    ...buildComplexObj(),
  };
